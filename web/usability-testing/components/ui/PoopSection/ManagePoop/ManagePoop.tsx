...
import { SETUP_POOP_VAULT } from "@cadence/transactions/Poop/transaction.setup-poop-vault"
import { IS_POOP_VAULT_INITIALIZED } from "@cadence/scripts/Poop/script.is-poop-vault-initialized"
import { GET_POOP_BALANCE } from "@cadence/scripts/Poop/script.get-balance"
import { GET_POOP_TOTAL_SUPPLY } from "@cadence/scripts/Poop/script.get-total-supply"
import { POOP_CREATE_NEW_MINTER } from "@cadence/transactions/Poop/administrator/transaction.create-new-minter"
import { POOP_MINT_TOKENS } from "@cadence/transactions/Poop/administrator/transaction.mint-tokens"
...
const ManageEmptyPotionBottle: FC<Props> = ({ id, title, user }) => {
  ...
  const isVaultInitialized = async () => {
    try {
      let response = await query({
        cadence: IS_POOP_VAULT_INITIALIZED,
  ...
  const createNewMinter = async () => {
    try {
      const res = await send([
        transaction(POOP_CREATE_NEW_MINTER),
        payer(authz),
        proposer(authz),
        authorizations([authz]),
        limit(9999),
